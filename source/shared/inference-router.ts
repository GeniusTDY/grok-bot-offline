export const SAND_INFERENCE_PROVIDERS = ["cursor", "claude-code", "codex", "openrouter", "proxy-gateway"] as const;
...
export function emptySandInferenceRouterUsage(): SandInferenceRouterUsage {
  ...
  return { schemaVersion: 1, providers: { cursor: empty(), "claude-code": empty(), codex: empty(), openrouter: empty(), "proxy-gateway": empty() } };
}
